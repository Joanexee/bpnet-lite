...
class DataGenerator(torch.utils.data.Dataset):
	...
	def __init__(self, sequences, signals, controls=None, p=None, in_window=2114, 
		...
		# If reverse complement, double the dataset by concatenating 
		# original and flipped versions
...
def PeakGenerator(peaks, negatives, sequences, signals, controls=None,
	...
	n_peak = len(seq_peak)
	n_bg_total = len(seq_bg)
	
	# Sample backgrounds according to negative_ratio
	n_bg_sample = int(n_peak * negative_ratio)
	
	if n_bg_sample > n_bg_total:
		print(f"Warning: Requested {n_bg_sample} background sequences but only {n_bg_total} available.")
		print(f"Using all {n_bg_total} background sequences.")
		n_bg_sample = n_bg_total
	
	# Sample background indices
	rng = numpy.random.RandomState(random_state)
	bg_indices = rng.choice(n_bg_total, size=n_bg_sample, replace=False)
	
	# Select sampled backgrounds
	seq_bg_sampled = seq_bg[bg_indices]
	signal_bg_sampled = signal_bg[bg_indices]

	# Concatenate peaks and sampled backgrounds
	seq = torch.cat([seq_peak, seq_bg_sampled], dim=0)
	sig = torch.cat([signal_peak, signal_bg_sampled], dim=0)
	...
	if reverse_complement:
		sequences_rc = torch.flip(seq, [1, 2])  # flip both nucleotide and position dims
		signals_rc = torch.flip(sig, [1, 2])  # flip both strand and position dims
		
		sequences = torch.cat([seq, sequences_rc], dim=0)
		signals = torch.cat([sig, signals_rc], dim=0)
		
		if controls is not None:
			controls_rc = torch.flip(con, [1, 2])
			controls = torch.cat([con, controls_rc], dim=0)
		else:
			controls = None
	else:
		sequences = seq
	...
	# force shuffle
	# indices = torch.randperm(len(seq))
	# seq = seq[indices]
	# sig = sig[indices]
	...
	if verbose:
		print(f"\n{n_peak} peaks and {n_bg_sample} negatives (sampled from {n_bg_total}) extracted.")
		print(f"Negative ratio: {negative_ratio} ({n_bg_sample}/{n_peak} = {n_bg_sample/n_peak:.3f})")
		if reverse_complement:
			print(f"With reverse complement augmentation, total training samples: {2 * (n_peak + n_bg_sample)}")
		else:
			print(f"Total training samples: {n_peak + n_bg_sample}\n")
